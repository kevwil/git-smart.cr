...
GitSmart.register "smart-merge" do |repo, args|
  ...
  if merge_sha == head
    ...
    !p "Branch '#{merge_target}' has diverged by #{new_commits_on_merge_target.length} commit#{"s" if new_commits_on_merge_target.length != 1}. Merging in."
    ...
    if head == merge_base
      ...
      !p "Branch '#{current_branch}' has #{new_commits_on_branch.length} new commit#{"s" if new_commits_on_merge_target.length != 1} since "#{merge_target}" diverged."
    end
