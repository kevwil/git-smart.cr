...
GitSmart.register "smart-pull" do |repo, args|
  ...
  if head == remote
    !p "Neither your local branch '#{branch}', nor the remote branch '#{upstream_branch}' have moved on."
    ...
    if merge_base == remote
      !p "Remote branch '#{upstream_branch}' has not moved on."
      success "Already up-to-date"
    else
      ...
      if merge_base == head
        !p "Local branch '#{branch}' has not moved on. Fast-forwarding..."
